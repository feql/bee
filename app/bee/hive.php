...
function hive_run_get_connection($username, $password,$servername="localhost",$databasename="",$is_test=false,$port=3306){
    try {
        // tools_dump("connecting to : ",__FILE__,__LINE__,array(
        //     "servername" => $username,
        //     "username" => $servername,
        //     "password" => $password,
        //     "db" => $databasename,
...
function hive_run($sql,$connection){
    ...
    try{
        if(tools_startsWith($sql,"INSERT") == TRUE){
            ...
            // }
            //tools_dump("sql ",__FILE__,__LINE__,array($connection, $sql));
        ...
        ///tools_dumpx("exxaecption",__FILE__,__LINE__,array($e->getMessage(),$sql ));
        $msg = $e->getMessage() . " " . ((BEE_SHOW_SQL_ON_ERRORS == TRUE)? $sql : "");
        array_push($hive_response[BEE_EI], $msg);
    }catch(Exception $e){
        //tools_dumpx("exception 2 ",__FILE__,__LINE__,array($e->getMessage(),$sql ));
        $msg = $e->getMessage() . " " . ((BEE_SHOW_SQL_ON_ERRORS == TRUE)? $sql : "");
        array_push($hive_response[BEE_EI], $msg);
    }
...
function hive_run_secture_sqlization($sectures){
    ...
    foreach ($sectures as $section_name => $secture) {
        ...
        if(tools_startsWith($secture[0],"vcnn")){
            if(count($secture) < 2){
                var_dump("this one");
                var_dump($section_sqls);
                var_dump($section_name);
                var_dump($secture);
                exit(0);
            }
            array_push($section_sqls,hive_run_vcnn($section_name,$secture[1]));
            continue;
        }
...
function hive_run_ac($connection,$comb_name,$sections_sql){
    $sql  = "ALTER TABLE ".$comb_name." ADD " . $sections_sql;
    $hr_res = hive_run($sql,$connection);
    return $hr_res;
}
...
function db_read_indexed($conn,$sql,$show_errors){
    $config = array(
        "sql" => $sql,
        "conn" => $conn,
        "BEE_SHOW_SQL_ON_ERRORS" => $show_errors
    );
...
function hive_run_ensure_garden_exists($garden_structrue){
    ...
    if(count($hrgc_res[BEE_EI]) == 0){
        ...
        if(count($hrch_res[BEE_EI]) == 0){
            ...
            $connection = null;
            //tools_dump("connection closed for text db: ",__FILE__,__LINE__,$connection);
            unset($connection);
...
function hive_run_get_garden($structure,$connection){
    //tools_dumpx("debhive_run_get_garden ",__FILE__,__LINE__,$structure);
    ...
    //tools_dumpx("@1 segmentation_run res: ",__FILE__,__LINE__,$sr_res[BEE_EI]);
    $hasr_res = hive_after_segmentation_run($sr_res,$nectoroid,$structure,$connection);
    //tools_dumpx("@2 bot res: ",__FILE__,__LINE__,$hasr_res[BEE_EI]);
...
function hive_run_get_garden_query($nectoroid, $structure,$connection){
    //tools_dumpx("hive_run_get_garden_query ",__FILE__,__LINE__,$structure);
    $res = array(null,array(),$structure);
    $sr_res = segmentation_run($nectoroid,$structure,$connection);
    //tools_dumpx("@1 segmentation_run res: ",__FILE__,__LINE__,$sr_res[BEE_EI]);
    $hasr_res = hive_after_segmentation_run($sr_res,$nectoroid,$structure,$connection);
...
function hive_run_post_garden_data($nectoroid,$structure,$connection,$user_id,$prev_res=array()){
    $res = array(null,array(),$structure);
...
function hive_after_segmentation_run($segmentation_run_res,$nectoroid,$structure,$connection){
    ...
    if(count($sr_res[BEE_EI]) == 0){//when we dont have any errors
        ...
        //tools_dumpx("@3 production_run res: ",__FILE__,__LINE__,$pr_res);
            ...
            if($valueXt == "_index_" || $valueXt == "_indexk_"){
                ...
                if(!empty($indexing)){
                    ...
                    foreach ($indexing as $indexing_key => $indexing_path) {
                        ...
                        //probably there was an indexing configuration
                        if(count($bee_indexing_keys) > 0){
                            $temp_bee_indexing_keys_name =  str_replace(".","_", $indexing_path);
                            if(array_key_exists($temp_bee_indexing_keys_name, $bee_indexing_keys)){
                                $indexing_id_key =  $bee_indexing_keys[$temp_bee_indexing_keys_name];
                            }
                            //tools_dumpx("bee_indexing_keys: ",__FILE__,__LINE__,[$indexing_key, $indexing_path, $bee_indexing_keys, $indexing_id_key]);
                        }
                        ...
                        for ($i=0; $i < count($index_path_parts); $i++) {
                            ...
                            if($i+1 == count($index_path_parts)){
                                ...
                                if($prev == "none"){
                                    ...
                                    $temp_sov_keys = array(); //we dont need duplicates
                                    foreach ($value_source as $sovObj) {
                                        if(!in_array($sovObj[$indexing_id_key],$temp_sov_keys)){
                                            array_push($temp_sov_keys,$sovObj[$indexing_id_key]);
                            ...
                            if($singular_index == $index_path_part){
                                ...
                                if($prev == "none"){
                                    ...
                                    foreach ($value_source as $sovHolder) {
                                        ...
                                        if(!array_key_exists($sov[$indexing_id_key],$temp_sov_keys)){
                                            array_push($temp_sov_keys,$sov[$indexing_id_key]);
                                            array_push($temp_sov,$sov);
                                        }
                                    }
                                    //the refrence now becomes the new source of truth
                                ...
                                if($prev == "none"){
                                    //we are just beginig our travel
                                    //tools_dumpx("sityaloss: ",__FILE__,__LINE__,array($index_path_part,$value_source));
                                    //the value source contains an array at this location
                                    $temp_array = $value_source[$index_path_part];
                                    //every object in this array is a potential source of the value
                                    //we are looking for
...
function bee_hive_update($nectoroid,$structure,$connection,$user_id,$prev_res=array(),$current_raw_honey=null){
    ...
    if(array_key_exists("requisition", $bsp_res[BEE_RI]) && strpos($bsp_res[BEE_RI]["requisition"], "_fk__pst_post") ){
        // tools_dumpx("bee_hive_update segmenetation: ",__FILE__,__LINE__,$bsp_res[BEE_RI]["requisition"]);
    }
...
function bee_hive_run_uploads($bee){
    ...
    //logged in
    if($bee["BEE_USER"]["id"] != 0 || true){

        // tools_dumpx("BEE NAME: ",__FILE__,__LINE__,$bee);
        $hn = $bee["BEE_APP_NAME"];
        $path_to_temp_dir = BASE_ROOT ."/bee/temp_uploads/uf_" . $hn . "_" . $bee["BEE_USER"]["id"]."/";
        ...
        foreach ($_FILES as $file_key => $file_value) {
            ...
            $allowed_file_types = array("jpg","png","jpeg","gif","pdf","txt","mp3","mp4","xlsx","docx","xls","xlx","rtf","odt");
            //nyd
