...
function hive_run_reset_hive($post_nectoroid,$bee){
    ...
    foreach ($bee["BEE_GARDEN"]["hives"] as $hive_index => $hive_obj) {
        ...
        foreach ($hive_obj["hive_users"] as $hive_user_index => $hive_user_obj) {
            if( intval($hive_user_obj["user_id"]) == $user_id  && 
                $hive_user_obj["hive_name"] == $hive_name){
                $found = true;
                $stop = true;
                break;
            }
    ...
    if(count($cnx[BEE_EI]) == 0 ){
        ...
        //tools_dump("connection closed d: ",__FILE__,__LINE__,$connection);
    }else{
...
function hive_run_change_password($post_nectoroid,$bee){
    global $BEE_GLOBALS;
    global $countries_list;
    $res = array(null, array());
    
    //email
    $email = $post_nectoroid["_f_change"]["email"];
    $hive_name = $post_nectoroid["_f_change"]["app_name"];
    $hive_name = BEE_GARDEN . "_" . tools_sanitise_name($hive_name);
    $code = $post_nectoroid["_f_change"]["code"];
    $raw_password = $post_nectoroid["_f_change"]["password"];
    $password = password_hash($raw_password, PASSWORD_DEFAULT);
    //nyd 
    //validation
    ...
    foreach ($bee["BEE_GARDEN"]["hives"] as $hive_index => $hive_obj) {
        $stop = false;
        // var_dump([$hive_name,$hive_obj["hive_users"],$post_nectoroid["_f_change"]]);
        foreach ($hive_obj["hive_users"] as $hive_user_index => $hive_user_obj) {
            if( $hive_user_obj["hive_name"] == $hive_name &&
                $hive_user_obj["email"] == $email ){
                $found = true;
                $stop = true;
                break;
            }
    ...
    if($found == false){
        $res[BEE_EI] = array("Account not found");
        return $res;
    }
...
function hive_run_password_edit_multitenat($post_nectoroid,$bee){
    global $BEE_GLOBALS;
    global $countries_list;
    $res = array(null, array());
    
    //email
    $email = $post_nectoroid["_f_password_edit_multitenat"]["email"];
    $code = $post_nectoroid["_f_password_edit_multitenat"]["code"];
    $raw_password = $post_nectoroid["_f_password_edit_multitenat"]["password"];
    $password = password_hash($raw_password, PASSWORD_DEFAULT);
    //nyd 
    //validation
    $hive_name = "";
