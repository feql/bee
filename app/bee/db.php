...
function db_run($run_config){
    $sql = $run_config["sql"];
    $db_show_sql_on_error  = (isset($run_config["BEE_SHOW_SQL_ON_ERRORS"]))? $run_config["BEE_SHOW_SQL_ON_ERRORS"] : BEE_SHOW_SQL_ON_ERRORS;
...
function db_ac($conn,$table_name, $colm_sql){
    $config = array(
        "sql" => "ALTER TABLE ".$table_name." ADD " . $colm_sql,
        "conn" => $conn,
        "BEE_SHOW_SQL_ON_ERRORS" => BEE_SHOW_SQL_ON_ERRORS
    );
    $dear_db = db_run($config);
    return $dear_db;
}
