...
function hive_run_recover_hive($post_nectoroid,$bee){
    ...
    for ($i=0; $i < count($found_records) ; $i++) { 
        ...
        if(count($cnx[BEE_EI]) == 0 ){
            ...
            //tools_dump("connection closed b: ",__FILE__,__LINE__,$connection);
        }else{
...
function hive_run_forgot_password($post_nectoroid,$bee){
    global $BEE_GLOBALS;
    global $countries_list;
    $res = array(null, array());
    
    //email
    $email = $post_nectoroid["_f_forgot"]["email"];
    //nyd 
    //validation
    ...
    if($found == false){
        $res[BEE_EI] = array("User account not found");
        return $res;
    }
    ...
    for ($i=0; $i < count($found_records) ; $i++) { 
        ...
        if(count($cnx[BEE_EI]) == 0 ){
            ...
            $brg_res = bee_run_get($user_nector,$bee["BEE_HIVE_STRUCTURE"]["combs"],$connection);
            $res[BEE_EI] = array_merge($res[BEE_EI],$brg_res[BEE_EI]);
            $found_records[$i]["user"] = null;
            if(count($brg_res[BEE_EI])==0){
                $found_records[$i]["user"] = $brg_res[BEE_RI]["user"];
            }
