<?php
    ...
    function production_run($sqls,$connection){
        ...
        foreach ($sqls as $sql_index => $sql_group) {
            ...
            $sql = $sql_group["sql"];
            //tools_dumpx("sql ",__FILE__,__LINE__,$connection);
    ...
    function command_fk_modify($cmd, $prev_res, $foreing_value, $niddle = null){
        if(strpos($foreing_value, "@") !== false){
            //contains an index
            $ky_indx = explode("@",$foreing_value);
            $ky = $ky_indx[0];
            $indx = intval($ky_indx[1]);
            // if( $prev_res == null || $ky == null || $indx == null){
            //     tools_dumpx("cmdx",__FILE__,__LINE__,[
            //         $prev_res,
            //         $ky,
            //         $indx
            //     ]);
                ...
                foreach ($sql as $index => $cmd) {
                    ...
                    if (strpos($cmd, "'_fk_") !== false && strpos($cmd, "_kf_'") !== false ) {
                        ...
                        for ($i=0; $i < count($foreing_values); $i++) { 
                            ...
                            if(tools_startsWith($foreing_value,"_pst_post.")){
                                //we have to look into the post post to see whats up
                                $pst_post_res = $prev_res["_pst_post"];
                                $fvs = explode("_post.",$foreing_value);
                                $fv = $fvs[1];
                                $cmd = command_fk_modify($cmd, $pst_post_res, $fv);
                            }else{
                                $cmd = command_fk_modify($cmd, $prev_res, $foreing_value);
                            }
                            // if(strpos($foreing_value, "@") !== false){
                            //     //contains an index
                            //     $ky_indx = explode("@",$foreing_value);
                ...
                if (strpos($sql, "'_fk_") !== false && strpos($sql, "_kf_'") !== false ) {
                    ...
                    for ($i=0; $i < count($foreing_values); $i++) { 
                        ...
                        if(tools_startsWith($foreing_value,"_pst_post.")){
                            ...
                            $fvs = explode("_post.",$foreing_value);
                            $fv = $fvs[1];
                            $sql = command_fk_modify($sql, $pst_post_res, $fv);
                        }else{
                            $sql = command_fk_modify($sql, $prev_res, $foreing_value);
                        }
                        // if(strpos($foreing_value, "@") !== false){
                        //     //contains an index
                        //     $ky_indx = explode("@",$foreing_value);
                ...
                if (strpos($sql, "'_fk_") !== false && strpos($sql, "_kf_'") !== false ) {
                    ...
                    for ($i=0; $i < count($foreing_values); $i++) { 
                        ...
                        if(tools_startsWith($foreing_value,"_julz")){
                            ...
                            continue;
                        }else if(tools_startsWith($foreing_value,"_pst_post.")){
                            //we have to look into the post post to see whats up
                            $pst_post_res = $prev_res["_pst_post"];
                            $fvs = explode("_post.",$foreing_value);
                            $fv = $fvs[1];
                            // tools_dumpx("staffstarted",__FILE__,__LINE__,[$foreing_value, $pst_post_res, $fvs, $fv]);
                            $niddle = "'_fk_".$foreing_value."_kf_'";
                            $sql = command_fk_modify($sql, $pst_post_res, $fv, $niddle);
                        }else{
                            $sql = command_fk_modify($sql, $prev_res, $foreing_value);
                        }
                        // tools_dumpx("sql*sql",__FILE__,__LINE__,[$foreing_value, $sql]);
                        // if(strpos($foreing_value, "@") !== false){
                        //     //contains an index
                        //     $ky_indx = explode("@",$foreing_value);
                        //     $ky = $ky_indx[0];
