<?php
    ...
    function  packaging_run($raw_honeys,$nectoroid,$structure,$connection, $is__n = false){
        ...
        if(array_key_exists("_n",$raw_honeys) && count($raw_honeys["_n"]) > 0){
            // tools_dumpx("found here ",__FILE__,__LINE__,$raw_honeys);
            ...
            if(array_key_exists(BEE_RI,$raw_honeys) && count($raw_honeys[BEE_RI]) > 0){
                //empty the _n key
                $raw_honeys["_n"] = array();
                $pr_res2 = packaging_run($raw_honeys,$nectoroid,$structure,$connection,false);
                $honey = array_merge($honey, $pr_res2[BEE_RI]);
            } 
        }else{
            foreach ($raw_honeys as $raw_honey_index => $raw_honey_group) {

                if(is_int($raw_honey_index) == false){
                    continue;
                }
