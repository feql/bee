<?php
    ...
    function segmentation_run($nectoroid,$structure,$connection){
        ...
        foreach ($nectoroid as $root_node_name => $root_node) {
            ...
            if(tools_startsWith($root_node_name,"_")){
                ...
                //_index_
                if(tools_startsWith($root_node_name,"_indexconfig_")){
                    //tools_dumpx("@1xtu res: ",__FILE__,__LINE__,$root_node_name);
                    if(!array_key_exists("_indexconfig_",$whole_honey)){
                        $whole_honey["_index_"] = array();
                    }
                    $nn = str_replace("_indexconfig_","",$root_node_name);
                    $path = $root_node["path"];
                    $key = $root_node["key"];
                    $bee_indexing_keys_name =  str_replace(".","_", $path);
                    $bee_indexing_keys[$bee_indexing_keys_name] = $key;
                    $whole_honey["_index_"][$nn] = $path;
                    //tools_dumpx("@1xtu res: ",__FILE__,__LINE__,$whole_honey);
                }
    ...
    function segmentation_run_process($nectoroid,$config,$connection){
        ...
        foreach ($node as $node_key => $node_key_value) {
            ...
            //left joining
            //_use_ is appliad to refrence parent table without including that tables details in the results
            //dont use it on inner child queries, it wont come out well
            if(tools_startsWith($node_key,"_use_")){
                $join_node_name = str_replace("_use_","",$node_key);
                $from_attr_parts = explode(".",$node_key_value); //user.id
                $use_comb_name = $from_attr_parts[0];
                $from_attr = $use_comb_name . "_id";
                if(count($from_attr_parts) > 1){
                    $from_attr = $from_attr_parts[1];
                }
                
                //e.g INNER JOIN user ON visit.user_id = user.id
                ...
                $use_sql =" INNER JOIN ".$use_comb_name ." ON ".$comb_name.".".$from_attr."=".$use_comb_name.".id ";
                //tools_dumpx("_use_ sql",__FILE__,__LINE__,$use_sql);
                $res[BEE_RI]["temp_inner_join_sql"] .= $use_sql;
                continue;
            }
            ...
            if($node_key == "_gb" || $node_key == "_g"){
                ...
                foreach ($node_key_value as $gbi) { //group by item
                    if(tools_startsWith($gbi,"_fk_")){ //i should leave as is 
                        $gb_temp = str_replace("_fk_","",$gbi);
                        $str_temp = $str_temp .  $gb_temp . ", ";
                        continue;
                    }
                    if(strpos($gbi,".")>0){
                        $gbis = explode(".",$gbi);
            ...
            if($node_key == "_ob" || $node_key == "_od" || $node_key == "_o" || $node_key == "_d" ||  $node_key == "_ao" ||  $node_key == "_do"
                ...
                foreach ($node_key_value as $obi) { //oder by item
                    if(strpos($obi,".")>0){
                        $obis = explode(".",$obi);
                        $cnx = Inflect::singularize($obis[0]); 
                        $str_temp = $str_temp .  ($cnx.".".$obis[1]) . ", ";
                    }else{
                        $str_temp = $str_temp .  ($comb_name .".".$obi) . ", ";
                    }
                }
            ...
            //var_dump($node_key.",".$singular);
            $SWO = tools_startsWith($singular,"other_");
            //var_dump("SWO =".$SWO );
            $ON = ($SWO)?str_replace("other_","",$singular):$singular;
            //var_dump("ON =".$ON." cn=". $comb_name );
            //  //this is the original line
            $tempComb = "";
            if($ON != $comb_name){
                $tempComb = ($SWO)?$comb_name: $comb_name;
            }else{
                $tempComb = ($SWO)?$ON: $comb_name;
            }
            $kkey = (($SWO)?$ON:$singular)."_id";
            //var_dump("kkey =".$kkey );
            //var_dump("tempComb =".$tempComb );
            $parent_id_exists = array_key_exists($kkey, $hive_structure[$tempComb]);
            ...
            // if($comb_name == "product"){
            //     tools_dump("inv",__FILE__,__LINE__,[
            //         $comb_name, $SWO, $ON, $singular, $node_key
            //     ]);
            // }
    ...
    function segmentation_run_a($node,$config,$connection){
        ...
        if(is_array($node)){
            foreach ($node as $section_query) {
                ...
                if(is_string($temp_sec)){
                    $temp_sec = trim($temp_sec);
                    if(strlen($temp_sec) == 0){
                        continue; //this is an empty string
                    }
                }
        ...
        if(!array_key_exists($comb_name,$hive_structure) && BEE_STRICT_HIVE == false && !$SWO ){
            ...
            //var_dump($hive_structure);
            file_put_contents(BASE_ROOT."/bee/".$hive_structure[BEE_FNN], json_encode($hive_structure));
        }
        ...
        if($node == "*" || (is_array($node) && count($node) == 1 && trim($node[0]) == "*" )  ){
        ...
        }else if( is_string($node) && tools_startsWith(trim($node),"*") && strlen(trim($node)) > 1 || 
                ( is_array($node) && count($node) > 1 && trim($node[0]) == "*" )  ){
            //this takes care of _a: "* time_inserted"
            $sections = array("id");
            $extras = is_string($node)? explode(" ",$node): $node;

            //get all the cols for this table including the id and any extras
            foreach ($sectures as $section_name => $secture) {
                if(tools_startsWith($section_name,"_") || tools_startsWith($secture,"_")){
                    continue;
                }
                if(in_array("!".$section_name,$extras)){
                    //this means skip this column
                    continue;
                }
                array_push($sections,$section_name);
            }
            for ($i=0; $i < count($extras); $i++) { 
                $extra = trim($extras[$i]);
                if(strlen($extra) == 0 || $extra == "*"){
                    continue;
                }
                if(tools_startsWith($extra,"!") || in_array($extra,$sections) ){
                    //this means skip this column
                    continue;
                }
                array_push($sections,$extra);
            }
        }
        //if its empty
        if(count($node) == 0){
            ...
            //those that were not requested for
        }
        ...
        foreach ($sections as $section_name) {
            ...
            if(tools_startsWith($section_name,"_")){
                ...
                //DATE_FORMAT(FROM_UNIXTIME(visit.time_inserted), '%Y-%m-%d') as date_entry
                //" _time2date_time_inserted"
                if(tools_startsWith($section_name,"_time2date_") && !$worked_on){
                    $section_name = substr($section_name,strlen("_time2date_"));
                    $sectfxs = array();
                    $aspart = $section_name;
                    if(stripos($section_name,"->") > -1){
                        $sectfxs = explode("->",$section_name);
                        ...
                        array_push($sectfxs, $section_name);
                    }
                    ...
                    $exp = " " . $comb_name . "." . $sectfxs[0];
                    $temp_path_to = $path . BEE_SEP . $aspart;
                    $subsql = " DATE_FORMAT(FROM_UNIXTIME(" . $exp . "), '%Y-%m-%d') as " . $temp_path_to;
                    $do_this_instead = $subsql;
                    $fall_through = true;
                    //tools_dumpx("changint to dte ",__FILE__,__LINE__,$do_this_instead);
                    $fall_through = true;
                    $worked_on = true;
                }
    ...
    //so we can have several wheres [w1,w2,w3]
    ...
    function seg_fxr($comb_name,$right,$node_name){
        if(stripos($right,"_fx_") > -1){
            if($right ==  "_fx__ipaddress_" || $right == "_fx__ip_"){
                $right = "'". $_SERVER["REMOTE_ADDR"] . "'";
            }elseif (stripos($right,"_fx_x_") > -1){
                ...
                $right = str_replace("_fx_","",$right);//should not have comb or column names
                //tools_dump("seg_fxr ",__FILE__,__LINE__,$right);
            }
        }
        //$right = "_fxx_";
    ...
    function segmentation_run_where_entry($where_entry,$comb_name,$node_name){
        if(is_string($where_entry)){
            return $where_entry; //we donot process
        }
        $left = $where_entry[0];
        ...
        if($condition == "=" || $condition == "e" || $condition == "E"){
            ...
            if((is_string($val_right) && !is_int(stripos($right,"_fx_"))) || !is_numeric($val_right) && !is_int(stripos($right,"_fx_"))){
                $val_right = "'".$val_right."'";
            ...
            $val_left = (is_array($left))? segmentation_run_where_entry($left,$comb_name,$node_name) : (seg_fx($comb_name,$left,$node_name));
            $val_right = (is_array($right))? segmentation_run_where_entry($right,$comb_name,$node_name) : seg_fxr($comb_name,$right,$node_name);
            // tools_dump("wtf : ",__FILE__,__LINE__,array(
            //     "is_string" => is_string($val_right),
            //     "is_int" => 
            // ));
            if((is_string($val_right) && !is_int(stripos($right,"_fx_"))) || !is_numeric($val_right) && !is_int(stripos($right,"_fx_"))){
                $val_right = "'".$val_right."'";
        ...
        }elseif($condition == "BW" || $condition == "bw" || $condition == "ibw" || $condition == "IBW" || 
                $condition == "SW" || $condition == "sw" || $condition == "isw" || $condition == "ISW" ){
            ...
            $val_right = (is_array($right))? segmentation_run_where_entry($right,$comb_name,$node_name) : seg_fxr($comb_name,$right,$node_name);
            $len = strlen($val_right);
            $sql = $sql . " LEFT(" . $val_left . ", ".$len.") = '".$val_right."'";
        }elseif($condition == "EW" || $condition == "ew" || $condition == "iew" || $condition == "IEW" ){
            //starts with or begins with
            ...
            $val_right = (is_array($right))? segmentation_run_where_entry($right,$comb_name,$node_name) : seg_fxr($comb_name,$right,$node_name);
            $len = strlen($val_right);
            $sql = $sql . " RIGHT(" . $val_left . ", ".$len.") = '".$val_right."'";
        }
        return $sql;
    }
    ...
    function bee_segmentation_update_process_sqllize($node_name, $comb_name, $node, $structure,$user_id, $whole_honey,  $current_raw_honey = null){
        ...
        foreach ($node as $key => $value) {
            ...
            if(tools_startsWith($section_name,"_")){
                ...
                $cntrep = 1;
                //time stamp
                if(tools_startsWith($section_name,"_time_")){
                    $section_name = substr($section_name, strlen("_time_")); //str_replace("_time_","",$section_name,$cntrep);
                    $value = strtotime($value);
                    $handled = true;
                }
                //now
                if(tools_startsWith($section_name,"_now_")){
                    $section_name = substr($section_name, strlen("_now_"));  //str_replace("_now_","",$section_name,$cntrep);
                    $value = time();
                    $handled = true;
                }
                //now date
                if(tools_startsWith($section_name,"_date_")){
                    $section_name = substr($section_name, strlen("_date_")); //str_replace("_date_","",$section_name,$cntrep);
                    $value = date("Y-m-d"); 
                    $handled = true;
                }
                //random _rnd_
                if(tools_startsWith($section_name,"_rnd_")){
                    $section_name = substr($section_name, strlen("_rnd_")); //str_replace("_rnd_","",$section_name,$cntrep);
                ...
                if(tools_startsWith($section_name,"_fk_")){
                    ...
                    $section_name = substr($section_name, strlen("_fk_")); //str_replace("_fk_","",$section_name,$cntrep);
                    $value = "_fk_" . $value . "_kf_";
                    $handled = true;
                }
                //encrypt
                if(tools_startsWith($section_name,"_encrypt_")){
                    $section_name = substr($section_name, strlen("_encrypt_")); //str_replace("_encrypt_","",$section_name,$cntrep);
    ...
    function bee_segmentation_post_process_sqllize($node_name, $comb_name, $node, $structure,$user_id, $whole_honey){
        ...
        $values_sql = "";
        $wasFile = false;
        $included_segmentation_functions_paths = array();
        foreach ($node as $key => $value) {
            $section_name = $key;
            //detect special flags
            if(tools_startsWith($section_name,"_")){
                ...
                //ip address
                if(tools_startsWith($section_name,"_ipaddress_") || tools_startsWith($section_name,"_ip_") ){
                    if(tools_startsWith($section_name,"_ipaddress_")){
                        $section_name =  substr($section_name, strlen("_ipaddress_")); 
                    }elseif (tools_startsWith($section_name,"_ip_")) {
                        $section_name =  substr($section_name, strlen("_ip_")); 
                    }
                    $value = $_SERVER["REMOTE_ADDR"];
                    //tools_dumpx("working on _ipaddress_ : ",__FILE__,__LINE__,[$value,$section_name]);
                    $handled = true;
                }
                //_fk_
                if(tools_startsWith($section_name,"_fk_")){
                ...
                //     }
                    $section_name = substr($section_name, strlen("_fk_"));  //str_replace("_fk_","",$section_name);
                    $value = "_fk_" . $value . "_kf_";
                    $handled = true;
                }
                //function execution
                if(tools_startsWith($section_name,"_f_")){
                    $section_name = substr($section_name, strlen("_f_"));   //str_replace("_f_","",$section_name);
                    

                    //tools_dumpx("working on sg funcs : ",__FILE__,__LINE__,$structure);

                    $functions_name = $value;
                    //check if this function has path config in _f_sg "functions called at attribute segmentation"
                    //nyd for now all this functions must be put under the folder funcs
                    //untill we find a way of giving access to the whole _hive.json structure
                    $path = "funcs" . DIRECTORY_SEPARATOR; 
                    $fileToInclude = $path . $functions_name . ".php";
                    //include this functions config if its not alreday included
                    //nyd: note that this will fail for recursive operations since 
                    //we are declaing the array in this function, solution is to make it 
                    //a global value store
                    if(in_array($fileToInclude,$included_segmentation_functions_paths) == false){
                        //its not yet included, so we include it
                        include($fileToInclude);
                        array_push($included_segmentation_functions_paths,$fileToInclude);
                        //execute that function
                        $BEE_SG_HONEY = $res;
                        $THIS_NODE = $node;
                        //tools_dump("working on sg funcs : THIS_NODE ",__FILE__,__LINE__,$THIS_NODE);
                        $temp_sg = null;
                        $exec = "\$temp_sg = $functions_name();";
                        eval($exec);
                        $res[BEE_EI] = array_merge($res[BEE_EI], $temp_sg[BEE_EI]);
                        $value = $temp_sg[BEE_RI][$functions_name];
                        //tools_dump("working retxxx ",__FILE__,__LINE__,[$temp_sg,$THIS_NODE]);
                    }
                    $handled = true;
                }
                if($handled == false){
    ...
    function bee_segmentation_evaluate_file_value($section_name,$value,$comb_name,$structure){
        ...
        if (!file_exists($value)) {
            ...
            $final_destination = BASE_ROOT ."/bee/uploads/"; //the default uploads location
    ...
    function bee_segmentation_delete_child_sqls($csqls,$pids,$comb_name,$structure,$connection,$user_id,$finished=[]){
        // tools_dumpx("mm : ",__FILE__,__LINE__,$comb_name);
        $res = array(null,array(),$structure);
        foreach ($structure as $section_name => $section_def) {
            if(in_array($comb_name, $finished)){
                continue;
            }
            if(tools_startsWith($section_name,"_")){
                continue;
    ...
    function bee_segmentation_get_ids_tobe_deleted($res,$comb_name,$root_node_name,$where_array,$structure,$connection){
        ...
        if($comb_name == $root_node_name){
            if($tobe != null){
                ...
                $temp = $res[3];
                $tooTemp = $tobe["id"];
                array_push($temp, $tooTemp);
            }else{
                // tools_dumpx("stopejer",__FILE__,__LINE__,[$root_node_name, $bsgtd[BEE_RI]]);
            }
        }else{
            //an array of objects
