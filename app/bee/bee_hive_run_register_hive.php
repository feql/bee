    ...
    */
    ...
    if(count($hrgc_res[BEE_EI]) == 0){
        ...
        if(count($hrch_res[BEE_EI]) == 0){
            ...
            $connection = null;
            //tools_dump("connection closed c: ",__FILE__,__LINE__,$connection);
            unset($connection);
    ...
    if(count($res[BEE_EI])>0){
        ...
        $special_keys = ["app_name","name","email","country","phone_number","password","code","tenant_of","is_owner","status"];

        foreach ($post_nectoroid["_f_register"] as $key => $value) {
            if(in_array($key,$special_keys) == true){
                continue;
            }
            if(array_key_exists($key,$user_temp) == true){
                continue;
            }
            $user_temp[$key] = $value;
        }
        ...
        //tools_dumpx("admin creation results: ",__FILE__,__LINE__,$brp_res3);
        ...
        if(array_key_exists("seeds",$bee["BEE_HIVE_STRUCTURE"])){
            ...
            $seeds["configs"] = array(
                array(
                    ...
                    "company_name" =>  $post_nectoroid["_f_register"]["app_name"],
                    "is_active" =>  1,
                    "decimal_places" =>  2,
                    "logo" =>  "",
                    "city" => "My City",
