...
function hive_run_update_password($post_nectoroid,$bee){
    ...
    //tools_dumpx("BEE_USER: ",__FILE__,__LINE__, $bee);
    ...
    if($found == false){
        $res[BEE_EI] = array("Account was not found");
        return $res;
    }
    ...
    if($bee["BEE_HIVE_STRUCTURE"]["is_registration_public"] == true){
        ...
        $hrgc_res = hive_run_get_connection(BEE_USER_NAME, BEE_PASSWORD,BEE_SERVER_NAME,$hive_name,false,BEE_PORT);
    ...
    if(count($cnx[BEE_EI]) == 0 ){
        ...
        //tools_dump("connection closed e: ",__FILE__,__LINE__,$connection);
    }else{
...
function hive_run_update_username($post_nectoroid,$bee){
    global $BEE_GLOBALS;
    global $countries_list;
    $res = array(null, array(),$bee);
    ...
    //hash the password
    $current_email = $post_nectoroid["_f_username"]["current_email"];
    $new_email = $post_nectoroid["_f_username"]["new_email"];
    $current_password = $post_nectoroid["_f_username"]["current_password"];
    $code = $post_nectoroid["_f_username"]["code"];
    $status = $post_nectoroid["_f_username"]["status"];
    ...
    if($current_email != null){
        ...
        if($current_email != $email){
            $res[BEE_EI] = array("Current email is not the account holder, authorization check failed");
            return $res;
        }
    }else{
        $res[BEE_EI] = array("Please provide your current username");
        return $res;
    }
    ...
    foreach ($bee["BEE_GARDEN"]["hives"] as $hive_index => $hive_obj) {
        ...
        foreach ($hive_obj["hive_users"] as $hive_user_index => $hive_user_obj) {
            ...
            if($app_name == $hive_obj["hive_name"] && $hive_user_obj["email"] == $email ){
                //tools_dumpx("hive_name: ",__FILE__,__LINE__,$hive_user_obj);
                if (password_verify($current_password, $hive_user_obj["password"])) {
                    $found_hive = $hive_obj;
                    $user_id = $hive_user_obj["user_id"];
                    $found = true;
                    $stop = true;
    ...
    if(count($cnx[BEE_EI]) == 0 ){
        ...
        if(count($brg_res[BEE_EI])==0){
            $users = $brg_res[BEE_RI]["users"];
            if(count($users)==0){
                array_push($res[BEE_EI],"Account was not found or may not be active");
            }else{
                $user = $users[0];
                //tools_dump("user",__FILE__,__LINE__,$user);
                $whole_honey = array();
        ...
        $res[BEE_EI] = array("Failed to establish connection to " . $hive_name);
    }
